...
export default class Archives extends React.Component {
    ...
    showSaveQuoteButton() {
        if (this.props.embedded === false) {
            return (<SaveQuoteButton userID={this.props.userID} company={this.props.company} description={this.props.description} setCurrentQuote={this.props.setCurrentQuote} {...this.state}/>)
        }
    }
                        ...
                        <div className="col-sm-6 sidenav">
                            <div className="panel panel-primary">
                                <div className="panel-body">
                                    <h4>Select the protocols you need.</h4>
                                            <label className="checkbox-inline">
                                                <input name="HLS" onChange={this.handleInputChange} checked={this.state.HLS}  type="checkbox" />HLS (Recommended)
                                            </label>
                                            <label className="checkbox-inline">
                                                <input name="HDS" onChange={this.handleInputChange} checked={this.state.HDS}  type="checkbox"/>HDS
                                            </label>
                                            <label className="checkbox-inline">
                                                <input name="RTMP" onChange={this.handleInputChange} checked={this.state.RTMP}  type="checkbox"/>RTMP
                                            </label>
                                            <label className="checkbox-inline">
                                                <input name="MPEG_DASH" onChange={this.handleInputChange} checked={this.state.MPEG_DASH}  type="checkbox"/>MPEG-DASH
                                            </label>
                                            <br/>
                                            
                                            <br/>
                                            <h4>Select a contract term in years.</h4>
                                            <select name="contractTerm" onChange={this.handleInputChange} value={this.state.contractTerm}  className="selectpicker" id="contract_term" required title="Choose one of the following...">
                                                <option id="contract_term_1">1</option>
                                                <option id="contract_term_2">2</option>
                                                <option defaultValue id="contract_term_3">3</option>
                                            </select>
                                            <h4>Enter the number of streams you need each year.</h4>
                                            ...
                                            <h4>Select a support plan for your services.</h4>
                                            <select name="supportPlan" onChange={this.handleInputChange} value={this.state.supportPlan}  className="selectpicker" id="support_plan" required>
